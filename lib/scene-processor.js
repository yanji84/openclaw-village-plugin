import { isVillageSession, resolvePending } from "./helpers.js";
...
export function createSceneProcessor(ctx, callGatewayRpc) {
  ...
  api.registerTool((toolCtx) => {
    const sk = toolCtx?.sessionKey || toolCtx?.session?.key || "";
    if (!isVillageSession(sk)) return null;
    return {
  ...
  return async function processScene(conversationId, payload) {
    ...
    // Trust the hub payload — any tool it sends is valid for this world
    ctx.activeToolDefs.clear();
    for (const t of payload.tools || []) {
      if (t.name) ctx.activeToolDefs.set(t.name, t);
    }
    ...
    // Combine owner persona + extension prompts + world system prompt
    const parts = [];
    if (ctx.ownerPersona) parts.push(ctx.ownerPersona);
    for (const ext of ctx.extensions) {
      if (ext.prompt) parts.push(ext.prompt);
    }
    if (ctx.activeSystemPrompt) parts.push(ctx.activeSystemPrompt);
    globalThis.__ggbot_village_prompt__ = parts.join("\n\n") || "";
